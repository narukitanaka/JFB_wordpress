                    ...
                    <?php 
                      ...
                      if ($maker_post) {
                        if (is_array($maker_post) && isset($maker_post[0])) {
                          ...
                          // メーカーのURLを取得
                          if ($maker_id) {
                            $maker_url = get_permalink($maker_id) . '#sendmail';
                            // メーカーのmail-addressフィールドを確認
                            $maker_email = get_field('mail-address', $maker_id);
                          }
                        }
                      }
                      if (!empty($maker_email) && $maker_url): 
                    ?>
                      <a class="btn bgc-re" href="<?php echo esc_url($maker_url); ?>">
                        Contact the maker
